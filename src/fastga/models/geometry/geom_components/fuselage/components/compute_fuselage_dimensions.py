...
class ComputeFuselageGeometryCabinSizingH2(ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        # Cabin total length
        cabin_length = l_instr + l_pax + l_lug + l_tank + l_fuelcell
        # Calculate nose length
        if prop_layout == 3.0:  # engine located in nose
            lav = nacelle_length
        else:
        ...
        outputs["data:geometry:fuselage:gravimetric_index"] = gravimetric_efficiency  # additional code
...
class ComputeFuselageGeometryCabinSizingDorsalH2(ExplicitComponent):
    # TODO: Document equations. Cite sources
    """
    Geometry of fuselage - Cabin is sized based on layout (seats, aisle...) and additional rear
    length (Fixed Length).
    ...
    def setup(self):
        ...
        self.add_output("data:geometry:fuselage:radius", units="m")  # additional code
        self.add_output("data:geometry:fuselage:fuelcell:length", units="m")  # additional code
        self.add_output("data:geometry:fuselage:shell_thickness", units="m")
        self.add_output("data:geometry:fuselage:insulation_thickness", units="m")
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        # Cabin total length
        cabin_length = l_instr + l_pax + l_lug + l_fuelcell
        # Calculate nose length
        if prop_layout == 3.0:  # engine located in nose
            lav = nacelle_length
        else:
        ...
        outputs["data:geometry:fuselage:radius"] = r_i  # additional code
        outputs["data:geometry:fuselage:fuelcell:length"] = l_fuelcell  # additional code
        outputs["data:geometry:fuselage:shell_thickness"] = t_shell
        outputs["data:geometry:fuselage:insulation_thickness"] = t_ins


class ComputeFuselageGeometryCabinSizingForwardH2(ExplicitComponent):
    # TODO: Document equations. Cite sources
    """
    Geometry of fuselage - Cabin is sized based on layout (seats, aisle...) and additional rear
    length (Fixed Length).
