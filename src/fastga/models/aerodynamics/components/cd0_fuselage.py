...
@oad.RegisterSubmodel(SUBMODEL_CD0_FUSELAGE, "fastga.submodel.aerodynamics.fuselage.cd0.hydrogendorsal")
class Cd0FuselageHydrogenDorsal(ExplicitComponent):
    """
    Profile drag estimation for the fuselage
    ...
    def setup(self):
        ...
        self.add_input("data:geometry:fuselage:wet_area", val=np.nan, units="m**2")
        self.add_input("data:geometry:wing:area", val=np.nan, units="m**2")
        self.add_input("data:geometry:fuselage:a", val=np.nan, units="m") #additional code
        if self.options["low_speed_aero"]:
            self.add_input("data:aerodynamics:low_speed:unit_reynolds", val=np.nan, units="m**-1")
            self.add_output("data:aerodynamics:fuselage:low_speed:CD0")
        else:
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        wet_area_fus = inputs["data:geometry:fuselage:wet_area"]
        wing_area = inputs["data:geometry:wing:area"]
        storage_radius = inputs["data:geometry:fuselage:a"] #additional code
        if self.options["low_speed_aero"]:
            unit_reynolds = inputs["data:aerodynamics:low_speed:unit_reynolds"]
        else:
            unit_reynolds = inputs["data:aerodynamics:cruise:unit_reynolds"]
        ...
        # Hydrogen storage drag model as a blister (Gudmundsson p.718)
        cds = 0.04
        area_storage = math.pi * (2*storage_radius) ** 2
        cd0_storage = cds * area_storage / wing_area

        # Fuselage
        cd0_fuselage = cf_fus * ff_fus * wet_area_fus / wing_area + cd0_storage
        # Cockpit window (Gudmundsson p727)
        cd0_window = 0.002 * (height * width) / wing_area
